...
@Path("/users")
public class BadgeUserResource {

	private static final Logger logger = Logger
			.getLogger(BadgeUserResource.class);

	@GET
	@Path("/info")
	public String getServiceDescr() {
		return "Risorsa REST per la gestione degli utenti Badge";
	}
		...
		try {
			...
			if (override != null && override) {
				if (start == null) {
					...
					criteria = criteria.andEndGreaterThan(start).andStartLessThan(end);
				}
			} else {
				if (start != null) {
					criteria = criteria.andStartLessThanOrEqualTo(start);
				}
				if (end != null) {
					criteria = criteria.andEndGreaterThanOrEqualTo(end);
				}
			}

			List<CalendarEvent> calendarEvents = calendarEventMapper.selectByExample(example);
			if (calendarEvents != null && calendarEvents.size() > 0) {
				...
				response.setMessage("Risultati trovati: " + response.getNumResults());
				logger.info(methodName + ": Trovati " + events.size() + " eventi");
			} else {
	...
	public Response findById(@PathParam("id") Integer id) {
		...
		try {
			...
			if (event != null) {
				return Response.status(Response.Status.OK).entity(CalendarEventHelper.convertToEvent(event)).build();
			}
		} catch (Throwable th) {
		...
		return Response.status(Response.Status.NOT_FOUND).entity("Event con id:" + id + " non trovato").build();
	}
	...
	public Response create(Event newEvent) {
		...
		try {
			...
			CalendarEvent calendarEvent = CalendarEventHelper.convertToCalendarEvent(newEvent);
		...
		return Response.status(Response.Status.CREATED).entity("Evento creato").build();
	}
	...
	public Response deleteById(@PathParam("id") Integer id) {
		...
		try {
			...
			if (calendarEventMapper.deleteByPrimaryKey(id) != 1) {
				...
				return Response.status(Response.Status.NOT_FOUND).entity("Evento con id: " + id + " non trovato").build();
			}
			sqlSession.commit();
		} catch (Throwable th) {
		...
		return Response.status(Response.Status.OK).entity("Evento con id: " + id + " eliminato").build();
	}
}
